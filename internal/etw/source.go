...
func (e *EventSource) Open(config *config.Config) error {
	...
	trace.AddProvider(etw.WindowsKernelProcessGUID, false, WithKeywords(etw.ProcessKeywords), WithCaptureState())
	...
	for _, t := range e.traces {
		...
		for _, provider := range t.Providers {
			// Enable state information for supported providers
			if !provider.CaptureState {
				continue
			}
			if err := etw.CaptureProviderState(provider.GUID, t.startHandle); err != nil {
				log.Warnf("unable to capture provider state: %v", err)
			}
		}

...
func (s *sequenceState) initFSM() {
	...
	s.fsm.OnTransitioned(func(ctx context.Context, transition fsm.Transition) {
		...
		if s.maxSpan != 0 && s.isStateSchedulable(s.currentState()) {
			log.Debugf("scheduling max span deadline of %v for expression %s", s.maxSpan, s.expr(s.currentState()))
		...
		if transition.Trigger == matchTransition {
			log.Debugf("state trigger from expression [%s]", s.expr(transition.Source))
			...
			if span, ok := s.spanDeadlines[transition.Source]; ok {
				log.Debugf("stopped max span deadline for expression %s", s.expr(transition.Source))
				span.Stop()
...
func (s *sequenceState) expr(seqID fsm.State) string {
	return s.exprs[seqID.(int)]
}
...
func (s *sequenceState) addPartial(seqID int, e *kevent.Kevent, isOOO bool) {
	...
	if isOOO {
		e.AddMeta(kevent.RuleExpressionKey, seqID)
		e.AddMeta(kevent.RuleSequenceOutOfOrderKey, true)
	}

	log.Debugf("adding partial to sequence slot [%d] for expression %q: %s", seqID, s.expr(seqID), e)
...
func (s *sequenceState) scheduleMaxSpanDeadline(seqID fsm.State, maxSpan time.Duration) {
	t := time.AfterFunc(maxSpan, func() {
		...
		if inState {
			log.Debugf("max span of %v exceded for expression %s", maxSpan, s.expr(seqID))
...
func (s *sequenceState) runSequence(e *kevent.Kevent) bool {
	for i, expr := range s.seq.Expressions {
		...
		if !s.next(i) {
			...
			s.mu.RLock()
			if expr.IsEvaluable(e) && s.filter.RunSequence(e, i, s.partials, true) {
		...
		if isTerminal {
			s.mu.RLock()
			nseqs := len(s.partials)

			setMatch := func(idx int, e *kevent.Kevent) {
				s.mmu.Lock()
				defer s.mmu.Unlock()
				if s.matches[idx] == nil {
					s.matches[idx] = e
				}
			}

			for idx := 0; idx < nseqs+1; i++ {
				for _, outer := range s.partials[i] {
					for _, inner := range s.partials[i+1] {
						if filter.CompareSeqJoin(outer.SequenceBy(), inner.SequenceBy()) {
							setMatch(i, outer)
							setMatch(i+1, inner)
						}
					}
...
func (e *Engine) matchUnorderedPartials(f *compiledFilter) {
	f.ss.mu.Lock()
	...
	//			continue
	//		}
	//		matches := f.run(partial, n, false, false)
	//		// transition the state machine
	//		if matches {
	//			//seqID := partial.GetMetaAsString(kevent.RuleExpressionKey)
	//			//err := f.ss.matchTransition(seqID, partial)
	//			//if err != nil {
	//			//	matchTransitionErrors.Add(1)

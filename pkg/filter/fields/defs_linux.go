...
var fields = platformFields(map[Field]FieldInfo{
	...
	EvtSyscallID:     {EvtSyscallID, "raw architecture-specific syscall number", params.Uint32, []string{"evt.syscall_id = 59"}, nil, nil},
	...
	FilePathTarget:   {FilePathTarget, "destination path of a rename or similar source/target file operation", platformString(), []string{"file.path.target = '/tmp/renamed'"}, nil, nil},
	...
	NetUnixPath:      {NetUnixPath, "filesystem path of an AF_UNIX socket", platformString(), []string{"net.unix_path = '/tmp/app.sock'"}, nil, nil},

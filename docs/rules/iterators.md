...
The `ps.pe._sections` pseudo field yields all of the executable image PE sections. Available section segments are:
...
|`module.signature.exists` | Indicates if the frame module is signed |
|`module.signature.trusted` | Indicates if the frame module signature is trusted |

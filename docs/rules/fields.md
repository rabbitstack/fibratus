...
Each field evaluates to a specific **data type**, which determines:

* How it can be compared
* Which operators are valid
* Which functions can be applied

Understanding field types is essential for writing **correct and efficient rules**.
...
* File paths (`file.name`, `file.path`)
* Process names (`ps.name`, `ps.exe`)
* Registry keys (`registry.key.name`)
...
* Use case-insensitive operators or normalization functions:

```text id="dbrj1b"
lower(file.name) = 'cmd.exe'
...
* **Integers**: `1`, `42`, `1000`
* **Floating-point numbers**: `6.54`, `0.001`

```text id="i4g7ls"
ps.pid = 1234
thread.cpu.usage > 6.5
```

Numbers use **dot (`.`)** notation for decimals.

---

### Boolean

Boolean fields represent truth values:
...
IP fields represent **IPv4 addresses**:

```text id="1r9c3y"
net.dst.ip = 172.14.4.4
```

These fields support:

* Equality comparisons
* Range checks (depending on operators/functions available)

> 💡 IP values are not strings — they are parsed and compared as structured addresses.

---

## Field access patterns

Fields are accessed using **dot notation**, which reflects their hierarchical structure:
...
This hierarchy mirrors the underlying event schema and helps keep rules expressive and readable.

---

## Multi-valued fields

Some fields may contain **multiple values** (collections), such as:

* Registry `MULTI_SZ` values
* Certain argument lists

These fields are typically used with:
...
```text id="x3m8pd"
get_reg_value(registry.key.name) iin ('bad.dll', 'evil.dll')
```
...
Not all fields are present in every event. When a field is missing:

* Expressions may evaluate to `false`
* Functions may return empty or null-like values

It is good practice to **design rules defensively**, especially when dealing with:

* Optional arguments (`kevt.arg[...]`)
...
| Type    | Example                     | Notes                     |
| ------- | --------------------------- | ------------------------- |
| String  | `file.name endswith '.exe'` | Supports pattern matching |
| Number  | `ps.pid > 1000`             | Numeric comparison        |
| Boolean | `ps.is_system = false`      | Direct equality           |
| IP      | `net.dst.ip = 10.0.0.1`     | Structured comparison     |
...
```text id="o2g6rn"
lower(file.name) = 'powershell.exe'
```
...
### Match file extensions safely

```text id="4n1s8d"
file.name iendswith '.dll'
```

---

### Compare numeric thresholds
...
```text id="w9y2mv"
kevt.arg[exe] imatches '?:\\Windows\\System32\\.*'
```
...
* Always be aware of the **field type** before writing comparisons
* Prefer **case-insensitive operators** for user-controlled input
* Normalize strings when consistency matters
* Be cautious with **missing fields** in heterogeneous event streams
* Use functions to bridge gaps between incompatible types

---

## Summary

Fields are the **foundation of every rule** in Fibratus. Mastering their types and behavior allows you to:

* Write precise detections
* Reduce false positives
* Build robust multi-step correlations

---

If you want, I can also extend this with a **full field taxonomy (ps.*, file.*, net.*, registry.*, kevt.*)** and describe when each group is populated, which is usually very helpful for rule authors.
...
| :---        |    :----   |          :---: |
| evt.seq      | Monotonic event sequence number       | `kevt.seq > 666`   |
| evt.pid      | Process identifier generating the event       | `kevt.pid = 6`   |
| evt.tid      | Thread identifier generating the event       | `kevt.tid = 1024`   |
| evt.cpu      | Logical processor core where the event was generated       | `kevt.cpu = 2`   |
| evt.name      | Symbolical event name       | `kevt.name = 'CreateThread'`   |
| evt.category      | Category to which the event pertains      | `kevt.category = 'registry'`   |
| evt.desc      | Cursory event description      | `kevt.desc contains 'Creates'`   |
| evt.host      | Hostname on which the event was produced     | `kevt.host contains 'dev'`   |
| evt.nparams    | Number of event parameters     | `kevt.nparams > 2`   |
| evt.time      | Event timestamp as a time string      | `kevt.time = '17:05:32'`   |
| evt.time.h      | Hour within the day on which the event occurred      | `kevt.time.h = 23`   |
| evt.time.m      | Minute offset within the hour on which the event occurred      | `kevt.time.m = 54`   |
| evt.time.s      | Second offset within the minute on which the event occurred      | `kevt.time.s = 0`   |
| evt.time.ns     | Nanoseconds specified by the event timestamp      | `kevt.time.ns > 1591191629102337000`   |
| evt.date       | Event timestamp as a date string      | `kevt.date = '2018-03-03'`   |
| evt.date.d     | Day of the month on which the event occurred      | `kevt.date.d = 12`   |
| evt.date.m     | Month of the year on which the event occurred      | `kevt.date.m = 11`   |
| evt.date.y     | Year on which the event occurred      | `kevt.date.y = 2020`   |
| evt.date.tz    | Time zone associated with the event timestamp     | `kevt.date.tz = 'UTC'`   |
| evt.date.week    | Week number within the year on which the event occurred     | `kevt.date.week = 2`   |
| evt.date.weekday    | Week day on which the event occurred     | `kevt.date.weekday = 'Monday'`   |
| evt.arg[]    | Accesses a specific event parameter via internal name | `kevt.arg[exe] = 'C:\\Windows\\cmd.exe'`   |
